...
class Builder(object):
  ...
  def __jar_build(cls, name, gradle_root):
    version = run_quick('cat {name}-docker.yml', echo=False).stdout.strip()
    cmds = [
      './release/all.sh {version}'.format(version=version),
    ]
  ...
  def build_jars(self):
    ...
    """
    subsystems = [comp for comp in SUBSYSTEM_LIST if comp == 'halyard']
